...
public class ItemAttribute {
    ...
    public void saveToConfig(ConfigurationSection section) {
        ...
        if (maxHPFlat != 0) section.set("max-hp-flat", maxHPFlat);
        if (maxSPPercent != 0) section.set("max-sp-%", maxSPPercent);
        if (maxSPFlat != 0) section.set("max-sp-flat", maxSPFlat);
        if (hpRecovery != 0) section.set("hp-recovery", hpRecovery);
        if (spRecovery != 0) section.set("sp-recovery", spRecovery);
        ...
        if (refinePAtk != 0) section.set("refine-p-atk", refinePAtk);
        if (refineMAtk != 0) section.set("refine-m-atk", refineMAtk);
        if (pDefBonus != 0) section.set("p-def", pDefBonus);
        if (mDefBonus != 0) section.set("m-def", mDefBonus);
        if (refinePDef != 0) section.set("refine-p-def", refinePDef);
        if (refineMDef != 0) section.set("refine-m-def", refineMDef);
        ...
        // Cooldown/Delay
        if (skillCDPercent != 0) section.set("skill-cd-%", skillCDPercent);
        if (skillCDFlat != 0) section.set("skill-cd-flat", skillCDFlat);
        if (finalCDPercent != 0) section.set("final-cd-%", finalCDPercent);
        if (globalCDPercent != 0) section.set("global-cd-%", globalCDPercent);
        if (afterCastDelayPercent != 0) section.set("after-cast-delay-%", afterCastDelayPercent);
        if (afterCastDelayFlat != 0) section.set("after-cast-delay-flat", afterCastDelayFlat);
        if (preMotion != 0) section.set("pre-motion", preMotion);
        if (postMotion != 0) section.set("post-motion", postMotion);
        if (cancelMotion != 0) section.set("cancel-motion", cancelMotion);
        ...
        if (atkIntervalPercent != 0) section.set("atk-interval-%", atkIntervalPercent);

        // Crit
        if (crit != 0) section.set("crit", crit);
        if (critDmgPercent != 0) section.set("crit-dmg-%", critDmgPercent);
        if (finalCritDmgPercent != 0) section.set("final-crit-dmg-%", finalCritDmgPercent);
        if (perfectHit != 0) section.set("perfect-hit", perfectHit);
        if (critRes != 0) section.set("crit-res", critRes);
        if (critDmgResPercent != 0) section.set("crit-dmg-res-%", critDmgResPercent);
        if (perfectDodge != 0) section.set("perfect-dodge", perfectDodge);
        ...
        if (healingFlat != 0) section.set("heal-flat", healingFlat);
        if (healingReceivedPercent != 0) section.set("heal-received-%", healingReceivedPercent);
        if (healingReceivedFlat != 0) section.set("heal-received-flat", healingReceivedFlat);
